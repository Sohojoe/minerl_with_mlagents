...
def main(argv=None, inject_create_environment_factory=None):
    try:
        print(
            """
    ...
    if num_runs == 1:
        ...
        run_training(0, run_seed, options, Queue(), inject_create_environment_factory)
    else:
        for i in range(num_runs):
            if seed == -1:
                run_seed = np.random.randint(0, 10000)
            process_queue = Queue()
            p = Process(target=run_training, args=(i, run_seed, options, process_queue, inject_create_environment_factory))
            jobs.append(p)
            p.start()
